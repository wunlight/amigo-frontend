...
function SaleDetails({ selectedProducts, selectedServices, totalSale, onSubmit, isSubmitting }: SaleDetailsProps) {
  ...
  return (
    <div className="flex flex-col gap-3 p-4 shrink-0 h-full w-80 bg-zinc-50">
      ...
      <div className="flex flex-col gap-3 py-3 min-h-0 max-h-full overflow-auto">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          {selectedServices.length === 0 ? (
            <div className="text-sm text-zinc-500 text-center py-4">No services selected</div>
          ) : (
            <div className="flex flex-col gap-2">
      ...
      <Button disabled={selectedProducts.length === 0 && selectedServices.length === 0 || isSubmitting} onClick={onSubmit}>
